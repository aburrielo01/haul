...
/**
 * País y moneda desde los que se leen las tiendas. El servidor está en Frankfurt,
 * y las tiendas Shopify con precios por país (Shopify Markets) le darían el precio
 * de Alemania. Estas cookies son las que guarda Shopify cuando alguien elige país.
 */
const SHOP_COUNTRY = (process.env.SHOP_COUNTRY || 'ES').toUpperCase();
const SHOP_CURRENCY = (process.env.SHOP_CURRENCY || 'EUR').toUpperCase();
const MARKET_COOKIE = `localization=${SHOP_COUNTRY}; cart_currency=${SHOP_CURRENCY}`;
...
function parsePrice(input, currencyHint) {
  ...
  // con separador de miles ("1.299,00") o sin él ("1299.00"): nunca cortar la cifra
...
async function viaShopify(rawUrl) {
  ...
  const [product, cart] = await Promise.all([
    fetchJson(base + '.js', 6000, { cookie: MARKET_COOKIE }),
    fetchJson(new URL(base).origin + '/cart.js', 4000, { cookie: MARKET_COOKIE }),
  ]);
  if (!product || !product.title || product.price === undefined) return null;

  // si el enlace apunta a una talla o color concretos, manda su precio
  const variant = variantId && Array.isArray(product.variants)
    ? product.variants.find((v) => String(v.id) === variantId)
    : null;
  const cents = Number(variant ? variant.price : product.price);
...
async function viaBrowser(url) {
  ...
  try {
    ...
    await context.addCookies([
      { name: 'localization', value: SHOP_COUNTRY, url },
      { name: 'cart_currency', value: SHOP_CURRENCY, url },
    ]).catch(() => {});
...
async function fetchJson(url, timeout = 7000, extraHeaders = {}) {
  const controller = new AbortController();
  const timer = setTimeout(() => controller.abort(), timeout);
  try {
    const res = await fetch(url, {
      signal: controller.signal,
      headers: { 'user-agent': 'Haul/2.0 (haul.app)', ...extraHeaders },
    });
